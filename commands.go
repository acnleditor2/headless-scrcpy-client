...
func runCommands(commands [][]string) {
	for _, command := range commands {
		...
		case "createuhiddevices":
			if len(command) == 4 {
				if config.Scrcpy.UhidKeyboardReportDesc == "" && command[1] != "" {
					if !createUhidDevice(command[1], 0x01, controlSocket) {
						return
					}
				}

				if config.Scrcpy.UhidMouseReportDesc == "" && command[2] != "" {
					if !createUhidDevice(command[2], 0x02, controlSocket) {
						return
					}
				}

				if config.Scrcpy.UhidGamepadReportDesc == "" && command[3] != "" {
					if !createUhidDevice(command[3], 0x03, controlSocket) {
						return
					}
				}
			} else {
		...
		case "key", "key2":
			if len(command) == 2 || len(command) == 5 {
				...
				if len(command) == 2 {
					if !sendKeyEventSdk(false, keycode, 0, 0) {
						return
					}

					if !sendKeyEventSdk(true, keycode, 0, 0) {
						return
					}
				} else {
					...
					if !sendKeyEventSdk(up, keycode, repeat, metaState) {
						return
					}
				}
			} else {
		...
		case "key3":
			if len(command) == 2 || len(command) == 3 {
				...
				if len(command) == 2 {
					if !sendKeyEventUhid(scancode, 0) {
						return
					}

					if scancode != 0 {
						if !sendKeyEventUhid(0, 0) {
							return
						}
					}
				} else {
					...
					if !sendKeyEventUhid(scancode, modifiers) {
						return
					}
				}
			} else {
		...
		case "touch":
			if len(command) == 5 {
				...
				if !sendTouchEvent(0, x, y, width, height) {
					return
				}

				if !sendTouchEvent(1, x, y, width, height) {
					return
				}
			} else {
				return
		...
		case "touchmove":
			if len(command) == 5 {
				...
				if !sendTouchEvent(2, x, y, width, height) {
					return
				}
			} else {
				return
		...
		case "mouseclick":
			if len(command) == 4 {
				...
				if !sendMouseEventUhid(x, y, button) {
					return
				}

				if !sendMouseEventUhid(0, 0, 0) {
					return
				}
				...
				if !sendMouseEventSdk(0, x, y, width, height, button) {
					return
				}

				if !sendMouseEventSdk(1, x, y, width, height, button) {
					return
				}
			} else {
				return
		...
		case "mousedown":
			if len(command) == 4 {
				...
				if !sendMouseEventUhid(x, y, getMouseButton(command[1])) {
					return
				}
			} else if len(command) == 6 {
				x, err := strconv.Atoi(command[2])
				...
				if !sendMouseEventSdk(0, x, y, width, height, getMouseButton(command[1])) {
					return
				}
			} else {
				return
		...
		case "mouseup":
			if len(command) == 1 {
				...
				if !sendMouseEventSdk(1, x, y, width, height, getMouseButton(command[1])) {
					return
				}
			} else {
				return
		...
		case "mousemove":
			if len(command) == 3 {
				...
				if !sendMouseEventSdk(2, x, y, width, height, getMouseButton(command[1])) {
					return
				}
			} else {
				return
		...
		case "scrollleft", "scrollright", "scrollup", "scrolldown":
			if len(command) == 3 && (command[0] == "scrollup" || command[0] == "scrolldown") {
				x, err := strconv.Atoi(command[1])
				if err != nil {
					return
				}
				...
				if !sendScrollEventUhid(command[0][6:], x, y) {
					return
				}
				...
				if !sendScrollEventSdk(command[0][6:], x, y, width, height) {
					return
				}
			} else {
				return
		...
		case "gamepadinput":
			if len(command) == 9 {
				...
				if !sendGamepadInput(leftX, leftY, rightX, rightY, leftTrigger, rightTrigger, buttons, dpad) {
					return
				}
			} else {
				return
		...
		case "getclipboard", "getclipboardcut":
			if len(command) == 1 {
				if getClipboard(command[0] == "getclipboardcut", nil, 0) != http.StatusNoContent {
					return
				}
			} else {
				return
		...
		case "setclipboard", "setclipboardbase64", "setclipboardbase64url", "setclipboardhex", "setclipboardpaste", "setclipboardpastebase64", "setclipboardpastebase64url", "setclipboardpastehex":
			if len(command) == 2 || len(command) == 3 || len(command) == 4 {
				...
				if !setClipboard(text, sequenceString, strings.HasPrefix(command[0], "setclipboardpaste"), timeout) {
					return
				}
			} else {
				return
		...
		case "adb":
			if len(command) > 1 && config.Adb.Enabled && config.Adb.Executable != "" {
				var args []string
				if config.Adb.Device == "usb" {
					args = append(config.Adb.Options, "-d")
				} else if config.Adb.Device == "tcpip" {
