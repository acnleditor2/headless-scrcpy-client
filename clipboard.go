...
func getClipboard(cut bool, text *string, timeout time.Duration) int {
	data := make([]byte, 2)
	data[0] = 0x08
	if cut {
		data[1] = 0x02
...
func setClipboard(text string, sequenceString string, paste bool, timeout time.Duration) bool {
	var sequence uint64
	var err error
...
func sendClipboardStream(w http.ResponseWriter, req *http.Request) {
