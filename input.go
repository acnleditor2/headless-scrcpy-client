...
func inputSdkInjectTouchEvent(action int, x int, y int, width int, height int) bool {
	data := make([]byte, 32)
	data[0] = 0x02
	data[1] = byte(action)
	data[2] = 0xFF
...
func inputSdkInjectMouseEvent(action int, x int, y int, width int, height int, button int) bool {
	data := make([]byte, 32)
	data[0] = 0x02
	data[1] = byte(action)
	data[2] = 0xFF
...
func inputUhidCreateDevice(reportDescString string, id int, controlSocket net.Conn) bool {
