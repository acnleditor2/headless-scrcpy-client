...
func createUhidDevice(reportDescString string, id int, controlSocket net.Conn) bool {
...
func sendKeyEventSdk(up bool, keycode int, repeat int, metaState int) bool {
	data := make([]byte, 14)
	if up {
		data[1] = 0x01
	}
...
func sendKeyEventUhid(scancode int, modifiers int) bool {
	data := make([]byte, 13)
	data[0] = 0x0D
	data[2] = 0x01
	data[4] = 0x08
...
func sendTouchEvent(action int, x int, y int, width int, height int) bool {
	data := make([]byte, 32)
	data[0] = 0x02
	data[1] = byte(action)
	data[2] = 0xFF
...
func sendMouseEventSdk(action int, x int, y int, width int, height int, button int) bool {
	data := make([]byte, 32)
	data[0] = 0x02
	data[1] = byte(action)
	data[2] = 0xFF
...
func sendMouseEventUhid(x int, y int, button int) bool {
	data := make([]byte, 9)
	data[0] = 0x0D
	data[2] = 0x02
	data[4] = 0x04
...
func sendScrollEventSdk(direction string, x int, y int, width int, height int) bool {
	data := make([]byte, 21)
	data[0] = 0x03
...
func sendScrollEventUhid(direction string, x int, y int) bool {
	data := make([]byte, 9)
	data[0] = 0x0D
	data[2] = 0x02
	data[4] = 0x04
...
func sendGamepadInput(leftX int, leftY int, rightX int, rightY int, leftTrigger int, rightTrigger int, buttons int, dpad int) bool {
	data := make([]byte, 20)
	data[0] = 0x0D
	data[2] = 0x03
	data[4] = 0x0F
...
func sendUhidKeyboardOutputStream(w http.ResponseWriter, req *http.Request) {
