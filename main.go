				...
				if connect {
					...
					if config.Scrcpy.Control {
						if config.Scrcpy.UhidKeyboardReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidKeyboardReportDesc, 0x01, controlSocket) {
						...
						if config.Scrcpy.UhidMouseReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidMouseReportDesc, 0x02, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
						...
						if config.Scrcpy.UhidGamepadReportDesc != "" {
							if !inputUhidCreateDevice(config.Scrcpy.UhidGamepadReportDesc, 0x03, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
