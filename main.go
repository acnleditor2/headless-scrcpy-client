...
func endpointHandler(w http.ResponseWriter, req *http.Request) {
	...
	case http.MethodGet:
		...
		if len(endpoint.Commands) > 0 {
			...
			case "videoStream":
				sendVideoStream(w, req, true)
			case "rawVideoStream":
				sendVideoStream(w, req, false)
			case "rgbVideoStream":
				if videoDecoderIsFfmpeg {
					sendFfmpegRgbVideoStream(w, req)
				} else {
					sendRgbVideoStream(w, req)
				}
			case "audioStream":
				sendAudioStream(w, req, true)
			case "rawAudioStream":
				sendAudioStream(w, req, false)
			case "clipboardStream":
				sendClipboardStream(w, req)
			case "uhidKeyboardOutputStream":
				sendUhidKeyboardOutputStream(w, req)
			case "clipboard":
				...
				var text string
				status := getClipboard(endpoint.ClipboardCut, &text, time.Duration(endpoint.ClipboardTimeout)*time.Millisecond)
				...
				if connect {
					...
					if config.Scrcpy.Control {
						if config.Scrcpy.UhidKeyboardReportDesc != "" {
							if !createUhidDevice(config.Scrcpy.UhidKeyboardReportDesc, 0x01, controlSocket) {
						...
						if config.Scrcpy.UhidMouseReportDesc != "" {
							if !createUhidDevice(config.Scrcpy.UhidMouseReportDesc, 0x02, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
						...
						if config.Scrcpy.UhidGamepadReportDesc != "" {
							if !createUhidDevice(config.Scrcpy.UhidGamepadReportDesc, 0x03, controlSocket) {
								go func() { connectionControlChannel <- false }()
								continue
							}
						}
